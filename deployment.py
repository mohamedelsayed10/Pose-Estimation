...
# import os
# import tempfile
# from prepoceesing import *
# from PIL import Image
# import matplotlib.pyplot as plt

# # Load models and PCA
# votingModelroll = joblib.load(r'C:\Users\mohamed elsayed\Desktop\New folder (5)\saved model\roll_model.joblib')
# votingModelpitch = joblib.load(r'C:\Users\mohamed elsayed\Desktop\New folder (5)\saved model\pitch_model.joblib')
# votingModelyaw = joblib.load(r'C:\Users\mohamed elsayed\Desktop\New folder (5)\saved model\yaw_model.joblib')
# pca = joblib.load(r'C:\Users\mohamed elsayed\Desktop\New folder (5)\saved model\pca_model.joblib')
...
# Load models and PCA from the saved files
...
# Define a function to process a video, predict orientation, and save the processed video
def process_video(video_path, votingModelroll, votingModelpitch, votingModelyaw, pca):
...
# Define a function to save a temporary file
def save_temp_file(uploaded_file, file_extension):
    temp_dir = tempfile.mkdtemp()
    temp_path = os.path.join(temp_dir, f"uploaded_file.{file_extension}")
    with open(temp_path, "wb") as temp_file:
...
def main():
    ...
    # Create a radio button to choose between image and video upload
    option = st.radio("What would you like to do?", ("Upload Image", "Upload Video"))

    if option == "Upload Image":
        # If image upload is chosen, provide an option to upload an image
        uploaded_file = st.file_uploader("Choose an image...", type=["jpg", "jpeg", "png"])
        file_extension = "jpg"

        # If an image is uploaded, process and display the results
        if uploaded_file is not None:
            temp_file_path = save_temp_file(uploaded_file, file_extension)
            image = cv2.imread(temp_file_path)
            roll, pitch, yaw, nose = predict_on_image(image, votingModelroll, votingModelpitch, votingModelyaw, pca)
        ...
        # If video upload is chosen, provide an option to upload a video
        uploaded_file = st.file_uploader("Choose a video...", type=["mp4"])
        if uploaded_file is not None:
            temp_video_path = save_temp_file(uploaded_file, "mp4")
            processed_video_path = process_video(temp_video_path, votingModelroll, votingModelpitch, votingModelyaw, pca)
...
# Run the Streamlit app if this script is the main module
if __name__ == "__main__":
    main()
